...
class CommandHandler:
    ...
    def handle_post_pdf_commands(session, message, clean_sender):
        ...
        try:
            ...
            # PRIORIDAD 1: Si estamos viendo historial y el usuario envía un dígito
            ...
            if message and message.lower() in ['menu', 'menú']:
                ...
                SessionManager.update_session_for_menu(session)
                return "✅ Tu RFI anterior fue procesado exitosamente.\n\n" + show_main_menu(), session
                ...
                try:
                    ...
                    prompt = PromptGenerator.get_step_prompt(new_session, 1)
            ...
            elif message and message.lower() in ['historial']:
                # Mostrar historial de RFIs
                try:
                    SessionManager.update_session_for_history(session)
                    # Guardar la sesión actualizada
                    from app.services.session_manager import SessionManager as SM
                    SM.save_session(clean_sender, session)
                ...
                return ("✅ *RFI Procesado Exitosamente*\n\nTu RFI anterior fue completado.\n\n"
                       "🔄 Escribe 'menu' para ver opciones\n📋 Escribe 'historial' para ver tus RFIs\n"
                       "📝 Escribe 'nuevo' para crear otro RFI"), session
